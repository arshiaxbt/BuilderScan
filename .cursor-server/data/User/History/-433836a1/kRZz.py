...
for target in target_trades:
    ...
    # Parse target time (make timezone aware)
    from datetime import timezone
    target_time = datetime.fromisoformat(target['time']).replace(tzinfo=timezone.utc)
    ...
    for trade in trades_data:
        trade_time_str = trade['created_time'].replace('Z', '+00:00')
