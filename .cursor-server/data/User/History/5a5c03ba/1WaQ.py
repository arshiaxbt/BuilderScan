#!/usr/bin/env python3
import requests
import time

URL = "https://www.itstheseason.christmas/api/advent/claim"
WALLETS = [
    "0x61bc0dBf7f960679124FF9Be26C7617ca10BfEeB",
    "0x55322A6A9ec077a20C9352a75274975e09Ce7f75",
    "0x7529fcdb6f71b13e262dae0ede0d7c9aca5a554c",
]

def try_claim(wallet, hour):
    payload = {"walletAddress": wallet, "hour": hour}
    headers = {"content-type": "application/json"}
    try:
        resp = requests.post(URL, json=payload, headers=headers, timeout=5)
        return resp.status_code, resp.text
    except Exception as e:
        return -1, str(e)

print("Checking hours 11-19...")
for hour in range(11, 20):
    print(f"\nHour {hour}:")
    for i, wallet in enumerate(WALLETS, 1):
        status, body = try_claim(wallet, hour)
        if status == 409:
            print(f"  Wallet #{i}: Already claimed ✅")
        elif status == 200:
            print(f"  Wallet #{i}: SUCCESS! 🎉")
        elif status == 403:
            print(f"  Wallet #{i}: Not unlocked yet")
        else:
            print(f"  Wallet #{i}: {status} - {body[:60]}")
        time.sleep(0.3)

