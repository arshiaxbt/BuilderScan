...
async def burst_attack(session: aiohttp.ClientSession, wallet: str, hour: int, log_file: str):
    ...
    for success, body in results:
        if isinstance(success, bool) and success:
            print(f"✓ Successfully claimed hour {hour} for {wallet[:10]}...")
            return True
        ...
        while True:
            ...
            if in_burst_window(now_utc):
                ...
                if last_burst_hour != submit_hour:
                    ...
                    if wallet:  # Only if wallet is configured
                        print(f"⚡ BURST ATTACK: Hour {submit_hour}, Wallet {wallet[:10]}..., Time: {utc_minute}:{utc_second:02d}")
